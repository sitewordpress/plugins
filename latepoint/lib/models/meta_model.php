...
class OsMetaModel extends OsModel{
  ...
  private static $encrypted_settings = array();
  ...
  public function get_by_key($meta_key, $object_id = false, $default = false){
    ...
    if($record){
      if(in_array($meta_key, self::$encrypted_settings)){
        return OsEncryptHelper::decrypt_meta_value($record->meta_value);
      }else{
  ...
  public function get_by_object_id($object_id = false, $default = []){
    ...
    if($records){
      ...
      foreach($records as $record){
        $value = in_array($record->meta_key, self::$encrypted_settings) ? OsEncryptHelper::decrypt_meta_value($record->meta_value) : $record->meta_value;

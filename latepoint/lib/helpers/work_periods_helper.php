    ...
    for($i=1; $i<=7; $i++){
      ...
      if(isset($working_periods_with_weekdays['day_'.$i])){
        ...
        foreach($working_periods_with_weekdays['day_'.$i] as $work_period){
          ...
          if(isset($args['agent_id']) && $args['agent_id'] && ((string)$work_period->agent_id !== (string)$args['agent_id'])){
            $work_period->agent_id = $args['agent_id'];
            $work_period->id = false;
          }
          if(isset($args['service_id']) && $args['service_id'] && ((string)$work_period->service_id !== (string)$args['service_id'])){
            $work_period->service_id = $args['service_id'];
            $work_period->id = false;
          }
          if(isset($args['location_id']) && $args['location_id'] && ((string)$work_period->location_id !== (string)$args['location_id'])){
